...
@section('content')
    ...
    <form action="{{ route('categories.update', $category->id) }}" method="post" >
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('title') is-invalid @enderror" value="{{$category->title}}"  name="title" id="title"  >                
        </div>
        <div class="mb-3">
            <label for="url_clean" class="form-label">Url neta</label>
            <input type="text" class="form-control" name="url_clean" value="{{$category->url_clean}}" id="url_clean" >
        </div>
